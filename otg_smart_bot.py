...
async def cmd_summary(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for msg in reversed(history):
        ...
        if target_user:
            if (
                msg.get("username") != target_user
                and msg.get("author", "").lower() != target_user[1:]
            ):
                continue
...
# === БЛОК ФОРМИРОВАНИЯ ПРОМПТА ===
    
    song_instruction = ""
    if song_mode:
        if style == 'j':
            song_instruction = (
                "\n\nВ самом конце ответа напиши короткий, но убойный текст для песни (рэп или панк-рок) по мотивам чата. "
                "ОБЯЗАТЕЛЬНО используй структуру с тегами:\n"
                "[Куплет] — 4 строки, жестко высмеивающие главных активистов по именам.\n"
                "[Припев] — 2-4 взрывные строки, обобщающие весь абсурд обсуждения.\n"
                "Соблюдай четкий ритм и рифму, чтобы это легко читалось как трек. Выдели блок эмодзи 🎸."
            )
        else:
            song_instruction = (
                "\n\nВ самом конце ответа напиши одно короткое красивое четверостишие с тегом [Финальный аккорд], "
                "которое поэтично резюмирует суть обсуждения. Выдели куплет эмодзи 🎵."
            )
    ...
    if style == "j":
        prompt = (
            ...
            f"⚠️ СТРОГИЕ ПРАВИЛА:\n"
            f"1. ВМЕСТО ИСПОЛЬЗОВАНИЯ ФОРМАТА 'Имя: цитата' или 'Имя сказал, что...'.\n"
            f"2. ГРУППИРУЙ ЛЮДЕЙ: вместо перечисления реплик пиши образами. Например: 'Пока Дмитрий и Антон мерялись импортозамещением, Сергей пытался обмануть систему за 11 рублей'.\n"
            ...
            f"Текст для анализа:\n\n{chat_text}{song_instruction}"
        )  # <-- ПРИКЛЕИЛИ ИНСТРУКЦИЮ ПЕСНИ
    else:
        prompt = (
            f"{limit_instruction}\n\n"
